          ...
          <div class="mt-2 flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-10">
            <div class="flex items-center">
              <input id="varnished-yes" type="radio" v-model="formData.varnished" value="Si"
                class="focus:ring-amber-500 h-4 w-4 text-amber-600 border border-gray-300" required>
...
<script>
import { ref, reactive, onMounted } from 'vue'; // Asegúrate de importar onMounted
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (!token) {
        router.push('/registro'); // Redirige al registro si no hay token
      }
    });
    ...
    const handleSubmit = () => {
      if (validateForm()) {
        ...
        handleCalculatePrice();  // Aquí se debe llamar a handleCalculatePrice, no calculatePrice.
      }
    };
