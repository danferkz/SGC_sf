    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
      <div class="p-8 space-y-8">
        ...
        <div class="text-center">
          <button @click="confirmarOrden"
            class="btn btn-lg btn-accent hover:btn-accent-focus transition duration-300 ease-in-out transform hover:scale-105">
            Ordenar
          </button>
        </div>
      </div>
...
export default {
  setup() {
    ...
    onMounted(async () => {
      try {
        const doorProductId = localStorage.getItem('product_id');
        const token = getToken();

        const response = await axios.get(
          `http://localhost:8000/api/products/product/${doorProductId}`,
          {
            headers: {
        ...
        producto.value = {
          ...
          tipoProducto: data.product_type === 'door' ? 'Puerta' : data.product_type,
